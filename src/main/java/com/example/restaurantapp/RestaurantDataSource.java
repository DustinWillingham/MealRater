...
public class RestaurantDataSource {
    ...
    public ArrayList<Restaurant> getRestraunts(String sortField, String sortOrder){
        ArrayList<Restaurant> restaurants = new ArrayList<Restaurant>();

        try{
            String query = "SELECT * FROM restaurants ORDER BY " + sortField + " " + sortOrder+ ";";
            Cursor cursor = database.rawQuery(query, null);
            ...
            while(!cursor.isAfterLast()) {
                ...
                newRestaurant.setName(cursor.getString(1));
                newRestaurant.setMeal(cursor.getString(1));
                newRestaurant.setRating(cursor.getFloat(0));
            }
        }
        catch(Exception e) {
